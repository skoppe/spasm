...
export default {
    jsExports: {
        ...
        glBufferData ( target, bufferLen, offset, usage) {
            let buffer = new Float32Array(spasm.base.buffer, offset, bufferLen);
        ...
        glGetAttribLocation ( program, len, offset) {
            return currentContext.getAttribLocation(objects[program],spasm.decodeStr(len,offset));
        },
        glGetShaderInfoLog ( shader ) {
        },
        glGetUniformLocation ( program, len, offset) {
            return addObject(currentContext.getUniformLocation(objects[program],spasm.decodeStr(len,offset)));
        },
        glLinkProgram ( program ) {
            currentContext.linkProgram(objects[program]);
        },
        glShaderSource ( shader, len, offset ) {
            currentContext.shaderSource(objects[shader], spasm.decodeStr(len,offset));
        },
        glTexImage2D ( target, level, internalformat, format, type, image) {
            currentContext.texImage2D(target, level, internalformat, format, type, dom.nodes[image]);
        },
        glTexParameteri ( target, pname, param ) {
            currentContext.texParameteri(target,pname,param);
        },
        glUniform1fv ( location, len, offset) {
            let buffer = new Float32Array(spasm.base.buffer, offset, len);
